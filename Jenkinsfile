         ...
         stage('Deploy to Kubernetes (Helm)') {
            ...
            steps {
                script {
                    ...
                    sh "helm upgrade --install helloworld ./helm-packages/springboot-hello-world-0.1.0.tgz --set image.repository=${DOCKER_REGISTRY}/${DOCKER_IMAGE} --set image.tag=${IMAGE_TAG} --values=./helm/charts/values.yaml"
                }
            }
        }
